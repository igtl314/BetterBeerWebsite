...
export default async function StorePage({
  ...
  // Mobile: category-filtered, APK-sorted, capped at 50
  const mobileFiltered = cat
    ? all.filter(i => normalizeCategory(i.product.ProductCategory) === cat)
    : all;
  const mobileRows = [...mobileFiltered]
    .sort((a, b) => (b.product.ProductApk ?? 0) - (a.product.ProductApk ?? 0))
    .slice(0, 50);
    ...
    <main>
      ...
      <div className="desk-only">
        ...
        <section style={{
          ...
          borderBottom: '1px solid var(--idx-line)',
          display: 'grid', gridTemplateColumns: '1fr 540px', gap: 64, alignItems: 'end',
        }}>
          <div>
            <div style={{
        ...
        <section style={{ padding: '0 56px 48px' }}>
          {sorted.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <thead>
                <tr>
                  <Th /><Th align="left">Beer / Brewer</Th>
                  <Th align="center">Category</Th><Th align="right">Vol</Th>
                  <Th align="right">ABV</Th><Th align="right">Price</Th>
                  <Th align="right">APK{sort === 'apk' ? ' ↓' : ''}</Th>
                  <Th align="right">Stock</Th>
                </tr>
              </thead>
              <tbody>
                {paginated.map((item) => <BeerRow key={item.ProductId} item={item} />)}
              </tbody>
            </table>
          )}
        </section>
        ...
        {totalPages > 1 && (
          <div style={{
            ...
            <PageControls id={id} currentPage={currentPage} totalPages={totalPages} cat={cat} sort={sort} instock={instock} />
          </div>
        )}
      ...
      <div className="mob-only mob-page">
        ...
        {/* Store header */}
        <section style={{ padding: '8px 20px 18px', borderBottom: '1px solid var(--idx-line)' }}>
          <div style={{
            fontFamily: 'var(--font-geist-mono)',
            fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--idx-fg-dim)',
          }}>
        ...
        <div style={{
          ...
          {[
            ['Beers', String(all.length), 'in catalogue'],
            ['Avg APK', avgApk > 0 ? avgApk.toFixed(2) : '—', 'value index'],
            ['Top APK', topApk > 0 ? topApk.toFixed(2) : '—', 'best in store'],
            ['Showing', String(mobileFiltered.length), cat ? `filtered by ${cat}` : 'sorted by APK'],
          ].map(([label, value, sub]) => (
            <div key={label} style={{ background: 'var(--idx-bg)', padding: '12px 16px' }}>
              <div style={{
                fontFamily: 'var(--font-geist-mono)',
        ...
        {/* Category filter chips — real Links, sticky on mobile */}
        <div
          className="mob-chips"
          style={{
            padding: '12px 20px',
        ...
        >
          <Link
            ...
            style={{
              ...
              background: !cat ? 'var(--idx-fg)' : 'transparent',
              color: !cat ? 'var(--idx-bg)' : 'var(--idx-fg-dim)',
              border: !cat ? 'none' : '1px solid var(--idx-line)',
            }}
          >
            All · {all.length}
          </Link>
          {categories.map(({ label, count }) => {
            ...
            return (
              <Link
                key={label}
                href={active ? `/stores/${id}` : `/stores/${id}?cat=${encodeURIComponent(label)}`}
                style={{
                  padding: '6px 12px', fontSize: 11,
                  fontFamily: 'var(--font-geist-mono)', letterSpacing: '0.04em',
                  whiteSpace: 'nowrap', textDecoration: 'none', flexShrink: 0,
        ...
        {/* Sort indicator */}
        <div style={{
          padding: '10px 20px', borderBottom: '1px solid var(--idx-line)',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <span style={{
            fontFamily: 'var(--font-geist-mono)', fontSize: 10,
            ...
            SHOWING {Math.min(50, mobileFiltered.length)} OF {mobileFiltered.length}
          </span>
          <span style={{
        ...
        {/* Beer rows */}
        <section>
          {mobileRows.map((item) => <MobileBeerRow key={item.ProductId} item={item} />)}
        </section>
      </div>
    </main>
  );
...
function FilterBarSkeleton({ count }: { count: number }) {
  return (
    <div style={{
      padding: '16px 56px', borderBottom: '1px solid var(--idx-line)',
      background: 'var(--idx-bg)', display: 'flex', alignItems: 'center', gap: 4,
    }}>
      <span style={{
...
function MobileBeerRow({ item }: { item: StockInfoWithProduct }) {
  const { product, Stock } = item;
  const apk = product.ProductApk ?? 0;
  const hasImage = !!product.ProductImageURL;
    ...
    <div style={{
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 10, color: 'var(--idx-fg-faint)', marginTop: 2 }}>
          {product.ProductVolume}ml · {product.ProductAlcohol.toFixed(1)}% · {product.ProductPrice.toFixed(2).replace('.', ',')} kr
        </div>
      </div>
      <div style={{ textAlign: 'right' }}>
        ...
        <span style={{
          display: 'inline-block', padding: '2px 7px', marginTop: 4,
...
function pageUrl(id: string, page: number, opts: { cat?: string; sort?: string; instock?: string }) {
...
function PageControls({ id, currentPage, totalPages, cat, sort, instock }: {
